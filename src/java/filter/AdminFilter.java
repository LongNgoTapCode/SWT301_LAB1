...
public class AdminFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        HttpSession session = req.getSession(false); // Tránh tạo session mới nếu không tồn tại

        // Kiểm tra session và user
        User u = (session != null) ? (User) session.getAttribute("user") : null;
        if (u == null || !"admin".equals(u.getRole())) { // Kiểm tra quyền dựa trên "role"
            // Lưu URL hiện tại để chuyển hướng lại sau khi login
